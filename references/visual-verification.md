...
Always use `--full` to capture the entire page, not just the viewport. Marketing pages
are tall — hero, features, testimonials, pricing, CTA, footer — and viewport-only
screenshots miss 70-80% of the content.
...
```
screenshots/<page-id>-v<iteration>-full.png        # Full page (source, tall)
screenshots/<page-id>-v<iteration>-001.jpg          # Chunk 1 (hero area)
screenshots/<page-id>-v<iteration>-002.jpg          # Chunk 2 (features area)
...
A full-page marketing screenshot is typically 1280x3000-6000px — far too tall for
Claude's vision to analyze effectively in one image. The resize script's `chunk` mode
slices it into viewport-sized pieces (default 900px tall) with overlap between chunks
so no element gets cut off at a boundary.
...
```
screenshots/<page-id>-v<iteration>-001.jpg   # Top of page (hero)
...
```
Read screenshots/<page-id>-v<iteration>-001.jpg   # Hero / above the fold
Read screenshots/<page-id>-v<iteration>-002.jpg   # Features / next section
Read screenshots/<page-id>-v<iteration>-003.jpg   # Testimonials / etc.
...
```
...
### Per-Section Checks
- **Hero chunk**: H1 visible? CTA button prominent? Value proposition clear?
- **Features chunk**: Feature cards/grid rendered? Icons showing? Descriptions present?
- **Social proof chunk**: Testimonials/logos displayed? Attribution visible?
- **Pricing chunk**: Tiers visible? Prices shown? Feature comparison readable?
- **CTA chunk**: Action button prominent? Supporting text present?
- **Footer chunk**: Links organized? Social icons showing? Copyright present?
...
python <skill-path>/scripts/state_manager.py \
  ...
  --result fail \
  --screenshot screenshots/<page-id>-v<iteration>-full.png \
  --notes "Chunk 3: testimonials section missing. Chunk 5: footer links misaligned."
```
...
python <skill-path>/scripts/state_manager.py \
  ...
  --page-id <page-id> \
  --status needs-review \
  --notes "After 3 iterations: minor spacing issue in footer remains"
```

## Regression Testing with Diff

After modifying any shared component (nav, footer, layout), use agent-browser's diff
...
agent-browser wait --load networkidle
agent-browser screenshot --full screenshots/home-baseline.png
...
agent-browser open http://localhost:3000/
agent-browser wait --load networkidle
agent-browser diff screenshot --baseline screenshots/home-baseline.png
```
...
For important pages, verify at three viewpoints. Use `--full` at each viewport:
...
reveals layout issues that desktop hides.
