...
in a page component, also add a corresponding entry to the manifest. Common image types:

| Image Type | Typical Dimensions | Aspect Ratio | Style Guidance |
|------------|--------------------|--------------| ---------------|
| Hero image | 1200×630 | 16:9 | Product screenshot or lifestyle, high impact |
| Feature illustration | 400×400 | 1:1 | Abstract/minimal illustration or icon |
| Testimonial avatar | 80×80 | 1:1 | Professional headshot, friendly |
| Logo/partner logo | 200×60 | variable | Clean logo on transparent background |
| OG social card | 1200×630 | 16:9 | Branded card with headline text |
| Blog thumbnail | 800×450 | 16:9 | Topical illustration or photo |
| Background pattern | 1920×1080 | 16:9 | Subtle texture or gradient |
...
- **Subject**: What is in the image ("a modern SaaS dashboard", "an abstract speed illustration")
...
**Good prompt:**
"A modern cloud infrastructure diagram showing connected servers, databases, and API
endpoints in an isometric view with clean lines and subtle shadows"

**Bad prompt:**
"Make a nice image for the features section"
...
```bash
python <skill-path>/scripts/generate_images.py --manifest image-manifest.json --only hero-main
```
...
```bash
python <skill-path>/scripts/generate_images.py --manifest image-manifest.json --style photorealistic
```
...
A typical marketing site (8-15 images) costs $0.50–$1.50 total.
...
#   /images/placeholders/hero-main.svg  →  /images/hero-main.webp
```
