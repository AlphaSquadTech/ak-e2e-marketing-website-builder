#!/usr/bin/env bash
# Preflight check for ak-e2e-marketing-website-builder
#
...
echo "=== Preflight Check: ak-e2e-marketing-website-builder ==="
echo ""
...
if [ "$SKIP_SKILLS" = true ]; then
  ...
  if command -v npx &>/dev/null; then
    ...
    for skill_spec in \
      ...
      "vercel-labs/agent-skills:web-design-guidelines"; do

      repo="${skill_spec%%:*}"
      skill_name="${skill_spec#*:}"
      # If no colon, skill_name equals repo (install all from that repo)
      if [ "$skill_name" = "$repo" ]; then
        display_name="$repo (all skills)"
        install_cmd="npx skills add $repo"
      else
        display_name="$repo → $skill_name"
        install_cmd="npx skills add $repo --skill $skill_name"
      fi
...
if [ "$SKIP_DEPS" = true ]; then
  ...
  if command -v python3 &>/dev/null; then
    ...
    fail "Python 3 not found (required for state_manager.py and resize_screenshot.py)"
  fi
